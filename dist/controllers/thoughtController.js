...
export const putThought = async (req, res) => {
    try {
        const thought = await Thought.updateOne({ _id: req.params.thoughtId }, { $set: { thoughtText: req.body.thoughtText } });
        res.json(thought);
    }
    catch (err) {
...
export const deleteThought = async (req, res) => {
    try {
        ...
        if (!thought) {
            ...
            await User.updateOne({ _id: thought.username }, { $pull: { thoughts: id } });
            res.json(thought);
        }
        ;
    }
...
export const deleteReaction = async (req, res) => {
    try {
        ...
        await Thought.updateOne({ _id: thoughtId }, { $pull: { reactions: { reactionId: reactionId } } });
